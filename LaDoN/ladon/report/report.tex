...
Deleting negative ties is important in its own right because it deletes negative ties, that will cause further polarization. But because of assortment, tie-deletion interacts with how random new ties are created. When assortment on opinions is high, the heuristic that "a friend of a friend is a friend" is very likely true. Triadic closure will therefore result in even more positive interactions, because similar to what is found in empirical networks, distance in the social network is highly indicative of distance in similarity. When randomness is high, the probability of connecting to a very dissimilar agent is much higher than through triadic closure. In other words, increasing randomness decreases assortment, which leads to more negative encounters.
This is also the reason for the correlation between the absolute value of opinions and the average path length when tie-deletion is high. Because the network co-evolves with opinions, when the average distance between opinions increase, the average distance in the network will increase as well. When opinions polarize, negative ties will tend to be deleted. Because of positive assortment, long-range ties will suffer especially when the network polarizes. This will make highways of information which connects distant parts of the network much less likely, which increases the average path length substantially. When the network depolarizes, the opposite happens. Some long range ties will be made by random, and because the network is depolarized, these will stay intact. This will make further connections more  
...
To further appreciate why tie-deletion facilitates cooperation, it is beneficial to examine representative simulations in more detail. Here we investigate one of the conditions, where threshold is relatively low (0.8), there is some negative social influence (0.1) and positive social influence (0.15). Conditions with the parameter values polarize in all cases when tie-deletion is low. So why do they reach consensus when the probability is high? The mechanism that facilitates reaching consensus is the same mechanism that facilitates cooperation in the computational biology literature; positive assortment \cite{boyd_coordinated_2010,dakin_dynamic_2018, pepper_mechanism_2002,santos_cooperation_2006}. In this model, positive assortment is achieved directly by deleting ties to dissimilar neighbors. But it is also achieved indirectly by creating new ties via triadic closure when the tie-deletion probability is high. It is likely to presume that any mechanism that will increase positive assortment in these kinds of models will decrease polarization. This is also why reaching consensus is more likely for simulations where most connections are formed via triadic closure instead of randomness. The reason for this effect is that random connections decrease positive assortment. As a network co-evolves with the opinions of its agents, communities of agents with similar opinions will be created. Just as we saw was the case in the empirical studies of dynamical social networks, distance in opinion space is a strong correlate for distance in opinion space \cite{kossinets_origins_2009}. In the model, the tie-deletion probability is the same for all agents. Because of this, closing ties via triadic closure will most likely result in positive interactions, because of the link between opinion and network distance. A friend of a friend is likely also a friend. Connecting randomly rather than via triadic closure will relatively often connect dissimilar. In other words, these interactions will be negative and result increased polarization. Counterintuitively, tie deletion in combination with triadic closure facilitates cooperation because it isolates groups, not despite it. This also explains why average path length and polarization correlates when tie deletion is high. When tie-deletion is high, the network behaves analogous to a spring. Because location in network space is correlated with location in opinion space, there will be less long-range connections as the opinions become more extreme. Long range ties will often be broken, because they are far apart in the network, and therefore very likely far apart in opinions. In these cases, the tension in the spring rises, and the network starts to be better characterized by as a network of two factions, connected mostly by a group of agents with values around 0. When polarization decreases, it is more plausible of these long range connections to stay intact and not be broken, which lowers the path length akin to the effect of classic small world networks \cite{watts_collective_1998}. When polarization increases, there will be less of these long-range ties, but there might also be fewer middlemen to facilitate cooperation across the aisle. This increases the path length considerably, because fewer highways of information exist in the system. 
In this view, it is also possible to give an alternative interpretation of what the probability of tie-deletion represents. The probability of tie-deletion can also be seen as how quickly the structure of the network is adjusted by opinions. In this interpretation, consensus can be achieved if the social network adjusts fast enough. Having faster adjustment of the structure of social interactions is critical, because the system is highly path dependent \cite{turner_paths_2018}.  When the network doesn’t adjust fast enough, agents will already have had several negative interactions, which will have pushed them further towards an extreme value of opinions. This will make them more likely to polarize other agents in the future.  
...
\begin{center} 
    ...
    \end{tabular} 
    \caption{Network evolutions for different levels of randomness. Columns show different values of randomness, $R$. Rows are different time-steps of the simulations, increasing in increments of 2500. Agents are colored by their opinion. Dark blue colors represent values close to -1, and lighter colors represent opinions close to 1.} 
    \label{fig:1} 
    \end{figure} 
\end{center} 
