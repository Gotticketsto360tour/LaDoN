...
def run_single_simulation(dictionary, run, target, target_dictionary) -> float:
    my_network = make_theoretical_network_by_seed(dictionary=dictionary, run=run)
...
if __name__ == "__main__":
    ...
    for name, network in NAME_DICTIONARY.items():
        ...
        joblib.dump(
            study, f"../analysis/data/optimization/{name}_study_no_theoretical.pkl"
        )
    with open(
        f"../analysis/data/optimization/best_optimization_results_theoretical.pkl",
        "wb",
    ) as handle:
